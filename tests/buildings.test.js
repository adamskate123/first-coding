...
import { BUILDING_PALETTES, shade } from '../src/render/palette.js';
import { ZONE_INFO, Z } from '../src/config.js';
...
function recipesFor(key, level) {
  return Array.from({ length: VARIANTS }, (_, v) => buildingRecipe(key, level, v));
}
...
test('colour ways are spread across the family, not stuck on one', () => {
  for (const { key, level } of CASES) {
    const used = new Set(recipesFor(key, level).map((r) => r.palette));
    assert.ok(used.size >= 3, `${key} level ${level}: only ${used.size} colour ways used`);
    for (const p of used) {
      assert.ok(p >= 0 && p < BUILDING_PALETTES[key].length, `${key}: palette ${p} out of range`);
    }
  }
});
...
test('every palette entry is a parseable hex colour', () => {
  for (const [family, ways] of Object.entries(BUILDING_PALETTES)) {
